...
const Physics = {
  ...
  createPlayer(x, y) {
    return {
      ...
      ropeSwing: false,
      facing: 1,
      state: 'idle',   // idle, walk, fall, swing, land
      stateTime: 0,
      coyoteTime: 0,
      landImpact: 0,
      // Limb animation
      legPhase: 0,
      armPhase: 0,
      // Upgrades applied externally
      upgrades: { ropeLen: 1, throwPower: 1, gripBoots: false, windCape: false, doubleThrow: false },
  ...
  updatePlayer(player, input, rope) {
    ...
    // Terrain collision X — push out of slopes
    const slopeAtX = World.getTerrainSlope(player.x);
    const terrAtX = World.getTerrainYSmooth(player.x);
    if (player.y + 2 > terrAtX && Math.abs(slopeAtX) > 0.5) {
      // On steep slope, push back
      player.x -= player.vx;
      player.vx *= -0.3;
    }
