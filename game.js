...
const Game = {
  ...
  init() {
    // Stop any running loop before reinitialising
    this.running = false;
    ...
    // Spawn player well above terrain so they land naturally on first frame
    const startX = 300;
    const terrainAtStart = World.getTerrainYSmooth(startX);
    const startY = terrainAtStart - 80;  // 80px above surface
    this.player = Physics.createPlayer(startX, startY);
    ...
    this.currentCampX = null;

    // Camera centred on player
    this.camX = startX - this.W / 2;
    this.camY = startY - this.H * 0.52;
  ...
  update() {
    ...
    // Death: fell far below terrain or off world
    if (this.player.y > World.getTerrainYSmooth(this.player.x) + 600) {
      this.triggerDeath();
      return;
    }
  ...
  loop() {
    ...
    this.update();
    this.render();
    // Only schedule next frame if still running (init() sets it false before restarting)
